    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        max_positions=[positions.argMax() for positions in livingGhostPositionDistributions]
        dists=[self.distancer.getDistance(pacmanPosition, max_position) for max_position in max_positions]
        nearest_ghost_index=dists.index(min(dists))
        nearest_ghost=max_positions[nearest_ghost_index]
        i=None
        curr=sys.maxsize
        for action in legal:
            next_pos=Actions.getSuccessor(pacmanPosition,action)
            dist=self.distancer.getDistance(next_pos,nearest_ghost)
            if dist<curr:
                i=action
                curr=dist
        return i
